...
class AutoRenewTrafficPermit(object):
    ...
    def renewTrafficPermit(self, account, hpzl, vid, issuedate):
        ...
        if code == 200:
            if '正在审核' in msg:
                ...
                crondate = datetime.datetime.strptime(issuedate, '%Y-%m-%d')
                crondate += datetime.timedelta(days=7)
    ...
    def ql_login(self) -> str:
        ...
        if os.path.isfile(path):
            ...
            headers = {'Authorization': 'Bearer {0}'.format(token), 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.71 Safari/537.36 Edg/94.0.992.38'}
